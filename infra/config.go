...
func (s *Server) setConfig() {
	...
	flag.String(fPort, ":8080", "http port for application")
	flag.String(fSPort, ":8081", "http port for service routes (/metrics, /pprof)")
	...
	flag.Visit(func(f *flag.Flag) {
		switch f.Name {
		case fPort:
			os.Setenv(envPrefix+"_HTTPD_MAIN_HOST_PORT", f.Value.String())
		case fSPort:
			os.Setenv(envPrefix+"_HTTPD_SERVICE_HOST_PORT", f.Value.String())
		case fLevel:
			os.Setenv(envPrefix+"_LOG_LEVEL", f.Value.String())
		case fLogFile:
			os.Setenv(envPrefix+"_LOG_FILE", f.Value.String())
		case fConsul:
			os.Setenv(envPrefix+"_CONSUL_URL", f.Value.String())
		case fPathConfig:
...
func (s *Server) consulRegister() {
		...
		&consulapi.AgentServiceRegistration{
			...
			Check: &consulapi.AgentServiceCheck{
				...
				HTTP: fmt.Sprintf("http://%s:%d/health",
					s.config.GetString("consul.address"), s.config.GetInt("consul.port")),
				Interval: "60s",
			},
	...
	s.log.Infof("consul register service success, serviceid=%s, consul.url=%s, consul.address=%s, consul.port=%d",
		s.config.GetString("consul.serviceid"), s.config.GetString("consul.url"), s.config.GetString("consul.address"),
